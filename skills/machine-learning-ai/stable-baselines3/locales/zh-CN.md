...
具有类似 scikit-learn 的 API 的可用于生产的强化学习算法（PPO、SAC、DQN、TD3、DDPG、A2C）。用于标准 RL 实验、快速原型设计和记录良好的算法实现。最适合在 Gymnasium 环境中进行单智能体强化学习。对于高性能并行训练、多代理系统或自定义矢量化环境，请改用 pufferlib。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `核心能力与原则` → `工作流程` → `资源` → `安装与配置`。
...
> 请使用“Stable Baselines3”处理【数据、任务定义与计算约束】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【模型、预测、指标或实验记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Stable Baselines3 完成准备数据与建模流程。我的材料是【数据、任务定义与计算约束】，结果请整理为【模型、预测、指标或实验记录】。”
- “请先检查我提供的【数据、任务定义与计算约束】是否足够，再用 Stable Baselines3 执行评估性能、偏差与复现条件；不要补造缺失信息。”
- “请把 Stable Baselines3 的处理过程做成可复核记录，交付【模型、预测、指标或实验记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Stable Baselines3** 完成“准备数据与建模流程” |
| 输入材料 | 数据、任务定义与计算约束 |
...
- [`references/callbacks.md`](../references/callbacks.md)：方法、规范或领域约束参考。
- [`references/custom_environments.md`](../references/custom_environments.md)：方法、规范或领域约束参考。
- [`references/vectorized_envs.md`](../references/vectorized_envs.md)：方法、规范或领域约束参考。

### 执行脚本（3）

- [`scripts/custom_env_template.py`](../scripts/custom_env_template.py)：可复用执行或验证脚本。
- [`scripts/evaluate_agent.py`](../scripts/evaluate_agent.py)：可复用执行或验证脚本。
- [`scripts/train_rl_agent.py`](../scripts/train_rl_agent.py)：可复用执行或验证脚本。
...
- [Pufferlib](../../pufferlib/locales/zh-CN.md)：针对速度和规模进行优化的高性能强化学习框架。当您需要快速并行训练、矢量化环境、多代理系统或与游戏环境（Atari、Procgen、NetHack）集成时使用。与标准实现相比，速度提高了 2-10 倍。对于快速原型设计或具有大量文档的标准算法实现，请使用 stable-baselines3。
...
- **本技能职责：**Stable Baselines3 聚焦于“构建、训练、评估和解释适用于科研数据的机器学习与人工智能模型”，负责完成“准备数据与建模流程；训练或调用机器学习模型；评估性能、偏差与复现条件”。
