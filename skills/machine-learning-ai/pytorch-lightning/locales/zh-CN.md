...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `核心能力与原则` → `工作流程` → `资源`。
...
> 请使用“Pytorch Lightning”处理【数据、任务定义与计算约束】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【模型、预测、指标或实验记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Pytorch Lightning 完成准备数据与建模流程。我的材料是【数据、任务定义与计算约束】，结果请整理为【模型、预测、指标或实验记录】。”
- “请先检查我提供的【数据、任务定义与计算约束】是否足够，再用 Pytorch Lightning 执行评估性能、偏差与复现条件；不要补造缺失信息。”
- “请把 Pytorch Lightning 的处理过程做成可复核记录，交付【模型、预测、指标或实验记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Pytorch Lightning** 完成“准备数据与建模流程” |
| 输入材料 | 数据、任务定义与计算约束 |
...
- [`references/data_module.md`](../references/data_module.md)：方法、规范或领域约束参考。
- [`references/distributed_training.md`](../references/distributed_training.md)：方法、规范或领域约束参考。
- [`references/lightning_module.md`](../references/lightning_module.md)：方法、规范或领域约束参考。
- [`references/logging.md`](../references/logging.md)：方法、规范或领域约束参考。
- [`references/trainer.md`](../references/trainer.md)：方法、规范或领域约束参考。

### 执行脚本（3）

- [`scripts/quick_trainer_setup.py`](../scripts/quick_trainer_setup.py)：可复用执行或验证脚本。
- [`scripts/template_datamodule.py`](../scripts/template_datamodule.py)：可复用执行或验证脚本。
- [`scripts/template_lightning_module.py`](../scripts/template_lightning_module.py)：可复用执行或验证脚本。
...
- **本技能职责：**Pytorch Lightning 聚焦于“构建、训练、评估和解释适用于科研数据的机器学习与人工智能模型”，负责完成“准备数据与建模流程；训练或调用机器学习模型；评估性能、偏差与复现条件”。
