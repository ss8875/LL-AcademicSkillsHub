...
采用结构化清单评审学术稿件或基金申请，覆盖研究方法、统计有效性、CONSORT/STROBE 等报告规范及建设性反馈。适合撰写正式同行评审意见和辅助稿件修订；若重点是主张与证据质量，可结合科学批判性思维技能；若需要量化评分，可结合学术评价技能。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `专题说明` → `工作流程` → `方法与策略` → `边界与问题排查` → `资源` → `最终检查清单`。
...
> 请使用“Peer Review”处理【研究材料、目标体裁、读者要求】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【学术文本或交流材料】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Peer Review 完成组织学术论证。我的材料是【研究材料、目标体裁、读者要求】，结果请整理为【学术文本或交流材料】。”
- “请先检查我提供的【研究材料、目标体裁、读者要求】是否足够，再用 Peer Review 执行按受众和体裁调整内容；不要补造缺失信息。”
- “请把 Peer Review 的处理过程做成可复核记录，交付【学术文本或交流材料】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Peer Review** 完成“组织学术论证” |
| 输入材料 | 研究材料、目标体裁、读者要求 |
...
- **本技能职责：**Peer Review 聚焦于“把证据、方法和结论组织为清楚、准确、适配目标读者的学术表达”，负责完成“组织学术论证；改善科学表达；按受众和体裁调整内容”。
