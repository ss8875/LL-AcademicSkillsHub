...
深度研究和写作工具的核心技能。用完整的段落撰写科学手稿（不要要点）。使用两阶段过程：(1) 使用研究查找列出带有要点的章节大纲，然后 (2) 转换为流畅的散文。 IMRAD 结构、引文（APA/AMA/温哥华）、图表、报告指南（CONSORT/STROBE/PRISMA），用于研究论文和期刊提交。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `专题说明` → `核心能力与原则` → `工作流程` → `协同与集成` → `参考资料`。
...
> 请使用“Scientific Writing”处理【研究材料、目标体裁、读者要求】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【学术文本或交流材料】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Scientific Writing 完成组织学术论证。我的材料是【研究材料、目标体裁、读者要求】，结果请整理为【学术文本或交流材料】。”
- “请先检查我提供的【研究材料、目标体裁、读者要求】是否足够，再用 Scientific Writing 执行按受众和体裁调整内容；不要补造缺失信息。”
- “请把 Scientific Writing 的处理过程做成可复核记录，交付【学术文本或交流材料】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Scientific Writing** 完成“组织学术论证” |
| 输入材料 | 研究材料、目标体裁、读者要求 |
...
- [`references/figures_tables.md`](../references/figures_tables.md)：方法、规范或领域约束参考。
- [`references/imrad_structure.md`](../references/imrad_structure.md)：方法、规范或领域约束参考。
- [`references/professional_report_formatting.md`](../references/professional_report_formatting.md)：方法、规范或领域约束参考。
- [`references/reporting_guidelines.md`](../references/reporting_guidelines.md)：方法、规范或领域约束参考。
- [`references/writing_principles.md`](../references/writing_principles.md)：方法、规范或领域约束参考。

### 模板与素材（3）

- [`assets/REPORT_FORMATTING_GUIDE.md`](../assets/REPORT_FORMATTING_GUIDE.md)：可复用模板、样式或示例素材。
- [`assets/scientific_report.sty`](../assets/scientific_report.sty)：可复用模板、样式或示例素材。
- [`assets/scientific_report_template.tex`](../assets/scientific_report_template.tex)：可复用模板、样式或示例素材。
...
- [Venue Templates](../../venue-templates/locales/zh-CN.md)：提供 Nature、Science、PLOS、IEEE、ACM 等期刊，NeurIPS、ICML、CVPR、CHI 等会议，以及研究海报和 NSF、NIH、DOE、DARPA 基金申请所需的 LaTeX 模板、格式要求与投稿指南。适合准备期刊论文、会议论文、研究海报或基金申请材料时使用。
...
- **本技能职责：**Scientific Writing 聚焦于“把证据、方法和结论组织为清楚、准确、适配目标读者的学术表达”，负责完成“组织学术论证；改善科学表达；按受众和体裁调整内容”。
