...
使用 Nano Banana Pro AI 和智能迭代细化创建专业的信息图表。使用 Gemini 3 Pro 进行质量审核。集成研究查找和网络搜索以获取准确的数据。支持 10 种信息图表类型、8 种行业样式和色盲安全调色板。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `快速开始` → `适用场景` → `协同与集成` → `任务结构` → `专题说明` → `参考资料` → `配置`。
...
> 请使用“Infographics”处理【数据、论点、目标媒介】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【图表、海报、幻灯片或图形文件】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Infographics 完成设计学术图表与版式。我的材料是【数据、论点、目标媒介】，结果请整理为【图表、海报、幻灯片或图形文件】。”
- “请先检查我提供的【数据、论点、目标媒介】是否足够，再用 Infographics 执行输出演示或出版素材；不要补造缺失信息。”
- “请把 Infographics 的处理过程做成可复核记录，交付【图表、海报、幻灯片或图形文件】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Infographics** 完成“设计学术图表与版式” |
| 输入材料 | 数据、论点、目标媒介 |
...
- [`references/design_principles.md`](../references/design_principles.md)：方法、规范或领域约束参考。
- [`references/infographic_types.md`](../references/infographic_types.md)：方法、规范或领域约束参考。

### 执行脚本（2）

- [`scripts/generate_infographic.py`](../scripts/generate_infographic.py)：可复用执行或验证脚本。
- [`scripts/generate_infographic_ai.py`](../scripts/generate_infographic_ai.py)：可复用执行或验证脚本。
...
- [Scientific Schematics](../../scientific-schematics/locales/zh-CN.md)：使用 Nano Banana 2 AI 和智能迭代细化创建出版质量的科学图表。使用 Gemini 3.1 Pro Preview 进行质量审查。仅当质量低于文档类型的阈值时才重新生成。专注于神经网络架构、系统图、流程图、生物路径和复杂的科学可视化。
...
- **本技能职责：**Infographics 聚焦于“把数据和论点转化为准确、易读、可编辑并适合传播的科研视觉材料”，负责完成“设计学术图表与版式；把数据转化为可读视觉；输出演示或出版素材”。
