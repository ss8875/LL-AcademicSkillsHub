...
使用 HTML/CSS 创建可导出为 PDF 或 PPTX 的研究海报，仅在明确需要 PowerPoint/PPTX 海报格式时使用；标准学术海报可优先选择 LaTeX Posters。支持现代网页式版面、响应式布局和可视化素材集成。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `专题说明` → `核心能力与原则` → `工作流程` → `任务结构` → `质量检查` → `应避免的问题`。
...
> 请使用“Pptx Posters”处理【数据、论点、目标媒介】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【图表、海报、幻灯片或图形文件】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Pptx Posters 完成设计学术图表与版式。我的材料是【数据、论点、目标媒介】，结果请整理为【图表、海报、幻灯片或图形文件】。”
- “请先检查我提供的【数据、论点、目标媒介】是否足够，再用 Pptx Posters 执行输出演示或出版素材；不要补造缺失信息。”
- “请把 Pptx Posters 的处理过程做成可复核记录，交付【图表、海报、幻灯片或图形文件】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Pptx Posters** 完成“设计学术图表与版式” |
| 输入材料 | 数据、论点、目标媒介 |
...
- [`assets/poster_html_template.html`](../assets/poster_html_template.html)：可复用模板、样式或示例素材。
...
- **本技能职责：**Pptx Posters 聚焦于“把数据和论点转化为准确、易读、可编辑并适合传播的科研视觉材料”，负责完成“设计学术图表与版式；把数据转化为可读视觉；输出演示或出版素材”。
