...
使用 AI 模型（FLUX、Nano Banana 2）生成或编辑图像。用于生成通用图像，包括照片、插图、艺术作品、视觉资产、概念艺术以及任何非技术图表或原理图的图像。对于流程图、电路、路径和技术图表，请使用科学原理图技能。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`适用场景` → `快速开始` → `安装与配置` → `模式与参数` → `常见用法` → `脚本与接口` → `示例与预览` → `边界与问题排查`。
...
> 请使用“Generate Image”处理【数据、论点、目标媒介】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【图表、海报、幻灯片或图形文件】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Generate Image 完成设计学术图表与版式。我的材料是【数据、论点、目标媒介】，结果请整理为【图表、海报、幻灯片或图形文件】。”
- “请先检查我提供的【数据、论点、目标媒介】是否足够，再用 Generate Image 执行输出演示或出版素材；不要补造缺失信息。”
- “请把 Generate Image 的处理过程做成可复核记录，交付【图表、海报、幻灯片或图形文件】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Generate Image** 完成“设计学术图表与版式” |
| 输入材料 | 数据、论点、目标媒介 |
...
- [`scripts/generate_image.py`](../scripts/generate_image.py)：可复用执行或验证脚本。
...
- **本技能职责：**Generate Image 聚焦于“把数据和论点转化为准确、易读、可编辑并适合传播的科研视觉材料”，负责完成“设计学术图表与版式；把数据转化为可读视觉；输出演示或出版素材”。
