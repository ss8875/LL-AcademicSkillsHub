...
使用 idc-index 从 NCI Imaging Data Commons 查询和下载公共癌症成像数据。用于访问大规模放射学（CT、MR、PET）和病理学数据集以进行 AI 培训或研究。无需身份验证。按元数据查询、在浏览器中可视化、检查许可证。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `专题说明` → `模式与参数` → `安装与配置` → `核心能力与原则` → `常见用法` → `最佳实践`。
...
> 请使用“Imaging Data Commons”处理【临床问题、医学数据或证据资料】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【证据摘要、分析结果或结构化记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Imaging Data Commons 完成整理临床与精准医学证据。我的材料是【临床问题、医学数据或证据资料】，结果请整理为【证据摘要、分析结果或结构化记录】。”
- “请先检查我提供的【临床问题、医学数据或证据资料】是否足够，再用 Imaging Data Commons 执行标注适用边界与风险；不要补造缺失信息。”
- “请把 Imaging Data Commons 的处理过程做成可复核记录，交付【证据摘要、分析结果或结构化记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Imaging Data Commons** 完成“整理临床与精准医学证据” |
| 输入材料 | 临床问题、医学数据或证据资料 |
...
- [`references/bigquery_guide.md`](../references/bigquery_guide.md)：方法、规范或领域约束参考。
- [`references/cli_guide.md`](../references/cli_guide.md)：方法、规范或领域约束参考。
- [`references/clinical_data_guide.md`](../references/clinical_data_guide.md)：方法、规范或领域约束参考。
- [`references/cloud_storage_guide.md`](../references/cloud_storage_guide.md)：方法、规范或领域约束参考。
- [`references/dicomweb_guide.md`](../references/dicomweb_guide.md)：方法、规范或领域约束参考。
- [`references/digital_pathology_guide.md`](../references/digital_pathology_guide.md)：方法、规范或领域约束参考。
- [`references/index_tables_guide.md`](../references/index_tables_guide.md)：方法、规范或领域约束参考。
- [`references/sql_patterns.md`](../references/sql_patterns.md)：方法、规范或领域约束参考。
- [`references/use_cases.md`](../references/use_cases.md)：方法、规范或领域约束参考。
...
- [Depmap](../../depmap/locales/zh-CN.md)：查询癌症依赖性图谱 (DepMap)，了解癌细胞系基因依赖性评分 (CRISPR Chronos)、药物敏感性数据和基因效应概况。用于识别癌症特定的脆弱性、合成致死相互作用以及验证肿瘤药物靶点。
...
- **本技能职责：**Imaging Data Commons 聚焦于“组织临床、影像和分子证据，为研究分析与精确医学决策提供结构化支持”，负责完成“整理临床与精准医学证据；处理受控医学数据；标注适用边界与风险”。
