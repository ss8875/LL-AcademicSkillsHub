...
以 LaTeX/PDF 格式为所有临床专业生成简洁（3-4 页）、重点突出的医疗计划。支持一般医疗、康复治疗、心理保健、慢性病管理、围手术期护理和疼痛管理。包括 SMART 目标框架、以最少文本引用的循证干预措施、法规遵从性 (HIPAA) 和专业格式。优先考虑简洁性和临床可操作性。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `专题说明` → `实践规范` → `核心能力与原则` → `最佳实践` → `质量检查` → `与其他技能集成`。
...
> 请使用“Treatment Plans”处理【临床问题、医学数据或证据资料】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【证据摘要、分析结果或结构化记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Treatment Plans 完成整理临床与精准医学证据。我的材料是【临床问题、医学数据或证据资料】，结果请整理为【证据摘要、分析结果或结构化记录】。”
- “请先检查我提供的【临床问题、医学数据或证据资料】是否足够，再用 Treatment Plans 执行标注适用边界与风险；不要补造缺失信息。”
- “请把 Treatment Plans 的处理过程做成可复核记录，交付【证据摘要、分析结果或结构化记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Treatment Plans** 完成“整理临床与精准医学证据” |
| 输入材料 | 临床问题、医学数据或证据资料 |
...
- 随技能打包 **20** 个文件，按用途完整列出如下。

### 参考资料（6）

- [`references/goal_setting_frameworks.md`](../references/goal_setting_frameworks.md)：方法、规范或领域约束参考。
- [`references/intervention_guidelines.md`](../references/intervention_guidelines.md)：方法、规范或领域约束参考。
...
- [`references/specialty_specific_guidelines.md`](../references/specialty_specific_guidelines.md)：方法、规范或领域约束参考。
- [`references/treatment_plan_standards.md`](../references/treatment_plan_standards.md)：方法、规范或领域约束参考。

### 执行脚本（4）

- [`scripts/check_completeness.py`](../scripts/check_completeness.py)：可复用执行或验证脚本。
- [`scripts/generate_template.py`](../scripts/generate_template.py)：可复用执行或验证脚本。
- [`scripts/timeline_generator.py`](../scripts/timeline_generator.py)：可复用执行或验证脚本。
- [`scripts/validate_treatment_plan.py`](../scripts/validate_treatment_plan.py)：可复用执行或验证脚本。

### 模板与素材（10）

- [`assets/chronic_disease_management_plan.tex`](../assets/chronic_disease_management_plan.tex)：可复用模板、样式或示例素材。
- [`assets/general_medical_treatment_plan.tex`](../assets/general_medical_treatment_plan.tex)：可复用模板、样式或示例素材。
- [`assets/medical_treatment_plan.sty`](../assets/medical_treatment_plan.sty)：可复用模板、样式或示例素材。
- [`assets/mental_health_treatment_plan.tex`](../assets/mental_health_treatment_plan.tex)：可复用模板、样式或示例素材。
- [`assets/one_page_treatment_plan.tex`](../assets/one_page_treatment_plan.tex)：可复用模板、样式或示例素材。
- [`assets/pain_management_plan.tex`](../assets/pain_management_plan.tex)：可复用模板、样式或示例素材。
- [`assets/perioperative_care_plan.tex`](../assets/perioperative_care_plan.tex)：可复用模板、样式或示例素材。
- [`assets/quality_checklist.md`](../assets/quality_checklist.md)：可复用模板、样式或示例素材。
- [`assets/rehabilitation_treatment_plan.tex`](../assets/rehabilitation_treatment_plan.tex)：可复用模板、样式或示例素材。
- [`assets/STYLING_QUICK_REFERENCE.md`](../assets/STYLING_QUICK_REFERENCE.md)：可复用模板、样式或示例素材。
...
- **本技能职责：**Treatment Plans 聚焦于“组织临床、影像和分子证据，为研究分析与精确医学决策提供结构化支持”，负责完成“整理临床与精准医学证据；处理受控医学数据；标注适用边界与风险”。
