...
查询 openFDA API 的药物、设备、不良事件、召回、监管提交（510k、PMA）、物质识别（UNII），用于 FDA 监管数据分析和安全研究。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `快速开始` → `专题说明` → `常见用法` → `产出要求` → `最佳实践` → `参考资料`。
...
> 请使用“Fda Database”处理【临床问题、医学数据或证据资料】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【证据摘要、分析结果或结构化记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Fda Database 完成整理临床与精准医学证据。我的材料是【临床问题、医学数据或证据资料】，结果请整理为【证据摘要、分析结果或结构化记录】。”
- “请先检查我提供的【临床问题、医学数据或证据资料】是否足够，再用 Fda Database 执行标注适用边界与风险；不要补造缺失信息。”
- “请把 Fda Database 的处理过程做成可复核记录，交付【证据摘要、分析结果或结构化记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Fda Database** 完成“整理临床与精准医学证据” |
| 输入材料 | 临床问题、医学数据或证据资料 |
...
- [`references/animal_veterinary.md`](../references/animal_veterinary.md)：方法、规范或领域约束参考。
- [`references/api_basics.md`](../references/api_basics.md)：方法、规范或领域约束参考。
- [`references/devices.md`](../references/devices.md)：方法、规范或领域约束参考。
- [`references/drugs.md`](../references/drugs.md)：方法、规范或领域约束参考。
- [`references/foods.md`](../references/foods.md)：方法、规范或领域约束参考。
- [`references/other.md`](../references/other.md)：方法、规范或领域约束参考。

### 执行脚本（2）

- [`scripts/fda_examples.py`](../scripts/fda_examples.py)：可复用执行或验证脚本。
- [`scripts/fda_query.py`](../scripts/fda_query.py)：可复用执行或验证脚本。
...
- **本技能职责：**Fda Database 聚焦于“组织临床、影像和分子证据，为研究分析与精确医学决策提供结构化支持”，负责完成“整理临床与精准医学证据；处理受控医学数据；标注适用边界与风险”。
