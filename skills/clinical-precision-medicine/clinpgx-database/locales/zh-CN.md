...
访问 ClinPGx 药物基因组学数据（PharmGKB 的后继者）。查询基因-药物相互作用、CPIC 指南、等位基因功能，以进行精准医疗和基因型指导的剂量决策。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `适用场景` → `安装与配置` → `核心能力与原则` → `工作流程` → `实践规范` → `专题说明` → `资源`。
...
> 请使用“Clinpgx Database”处理【临床问题、医学数据或证据资料】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【证据摘要、分析结果或结构化记录】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Clinpgx Database 完成整理临床与精准医学证据。我的材料是【临床问题、医学数据或证据资料】，结果请整理为【证据摘要、分析结果或结构化记录】。”
- “请先检查我提供的【临床问题、医学数据或证据资料】是否足够，再用 Clinpgx Database 执行标注适用边界与风险；不要补造缺失信息。”
- “请把 Clinpgx Database 的处理过程做成可复核记录，交付【证据摘要、分析结果或结构化记录】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Clinpgx Database** 完成“整理临床与精准医学证据” |
| 输入材料 | 临床问题、医学数据或证据资料 |
...
- [`scripts/query_clinpgx.py`](../scripts/query_clinpgx.py)：可复用执行或验证脚本。
...
- **本技能职责：**Clinpgx Database 聚焦于“组织临床、影像和分子证据，为研究分析与精确医学决策提供结构化支持”，负责完成“整理临床与精准医学证据；处理受控医学数据；标注适用边界与风险”。
