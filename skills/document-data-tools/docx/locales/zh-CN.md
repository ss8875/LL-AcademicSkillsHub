...
每当用户想要创建、读取、编辑或操作Word文档（ .docx文件）时，请使用此技能。触发器包括：任何提及“Word文档”、“Word文档”、“.docx”或要求生成具有目录、标题、页码或信笺抬头等格式的专业文档。也可用于从.docx文件中提取或重新组织内容，在文档中插入或替换图像， 在Word文件中执行查找和替换，处理跟踪的更改或注释，或将内容转换为精美的Word文档。如果用户要求提供“报告”、“备忘录”、“信件”、“模板”或类似的Word或.docx文件，请使用此技能。请勿用于PDF、电子表格、Google文档或与文档生成无关的一般编码任务。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `参考资料` → `专题说明` → `依赖`。
...
> 请使用“Docx”处理【文档、表格或数据文件】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【转换文件、结构化数据或校验报告】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Docx 完成读取与转换科研文档。我的材料是【文档、表格或数据文件】，结果请整理为【转换文件、结构化数据或校验报告】。”
- “请先检查我提供的【文档、表格或数据文件】是否足够，再用 Docx 执行保持版式、字段与来源可追溯；不要补造缺失信息。”
- “请把 Docx 的处理过程做成可复核记录，交付【转换文件、结构化数据或校验报告】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Docx** 完成“读取与转换科研文档” |
| 输入材料 | 文档、表格或数据文件 |
...
- 随技能打包 **60** 个文件，按用途完整列出如下。

### 执行脚本（59）

- [`scripts/__init__.py`](../scripts/__init__.py)：可复用执行或验证脚本。
- [`scripts/accept_changes.py`](../scripts/accept_changes.py)：可复用执行或验证脚本。
- [`scripts/comment.py`](../scripts/comment.py)：可复用执行或验证脚本。
...
- [`scripts/templates/comments.xml`](../scripts/templates/comments.xml)：可复用执行或验证脚本。
- [`scripts/templates/commentsExtended.xml`](../scripts/templates/commentsExtended.xml)：可复用执行或验证脚本。
- [`scripts/templates/commentsExtensible.xml`](../scripts/templates/commentsExtensible.xml)：可复用执行或验证脚本。
- [`scripts/templates/commentsIds.xml`](../scripts/templates/commentsIds.xml)：可复用执行或验证脚本。
- [`scripts/templates/people.xml`](../scripts/templates/people.xml)：可复用执行或验证脚本。
...
- **本技能职责：**Docx 聚焦于“读取、生成、转换和检查科研文档、表格、演示与结构化文件”，负责完成“读取与转换科研文档；提取结构化内容；保持版式、字段与来源可追溯”。
