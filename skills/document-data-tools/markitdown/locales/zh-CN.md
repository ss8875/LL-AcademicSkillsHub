...
将文件和Office文档转换为Markdown。支持PDF、DOCX、PPTX、XLSX、图像（带OCR ）、音频（带转录）、HTML、CSV、JSON、XML、ZIP、YouTube URL、EPub等。
...
本技能按“输入契约 → 执行 → 验证 → 交付”工作。原始指令的重点阅读路径为：`能力概览` → `专题说明` → `快速开始` → `核心能力与原则` → `模式与参数` → `常见用法` → `最佳实践` → `工作流程`。
...
> 请使用“Markitdown”处理【文档、表格或数据文件】。目标是【写明研究目标】；请遵守【时间范围、对象范围、格式或方法约束】，输出【转换文件、结构化数据或校验报告】，并列出关键步骤、证据来源、不确定性和需要人工确认的事项。

### 可直接改写的请求

- “请用 Markitdown 完成读取与转换科研文档。我的材料是【文档、表格或数据文件】，结果请整理为【转换文件、结构化数据或校验报告】。”
- “请先检查我提供的【文档、表格或数据文件】是否足够，再用 Markitdown 执行保持版式、字段与来源可追溯；不要补造缺失信息。”
- “请把 Markitdown 的处理过程做成可复核记录，交付【转换文件、结构化数据或校验报告】，同时标出假设、限制和下一步建议。”

## 5. 示例预览

| 环节 | 示例内容 |
|---|---|
| 任务目标 | 使用 **Markitdown** 完成“读取与转换科研文档” |
| 输入材料 | 文档、表格或数据文件 |
...
- [`references/file_formats.md`](../references/file_formats.md)：方法、规范或领域约束参考。

### 执行脚本（3）

- [`scripts/batch_convert.py`](../scripts/batch_convert.py)：可复用执行或验证脚本。
- [`scripts/convert_literature.py`](../scripts/convert_literature.py)：可复用执行或验证脚本。
- [`scripts/convert_with_ai.py`](../scripts/convert_with_ai.py)：可复用执行或验证脚本。

### 模板与素材（1）

- [`assets/example_usage.md`](../assets/example_usage.md)：可复用模板、样式或示例素材。
...
- **本技能职责：**Markitdown 聚焦于“读取、生成、转换和检查科研文档、表格、演示与结构化文件”，负责完成“读取与转换科研文档；提取结构化内容；保持版式、字段与来源可追溯”。
